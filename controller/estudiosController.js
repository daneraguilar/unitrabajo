...
module.exports = {
    guardar: function(req, res) {
        ...
        egresados.findOne({ _id: id }, function(err, cv) {
            ...
            estudio.save(function(err, e) {
                if (err) {
                    return res.json({
                        message: 'Error error guaradando estudio.',
                        err
                    });
                }
                ...
                cv.save(function(err, c) {
                    ...
                    if (err) {
                        return res.status(500).json({
                            message: 'Error error guaradando cv ',
                            err
                        });
                    }
    ...
    modificar: function(req, res) {
        ...
        estudios.findOne({ _id: id }, function(err, estudio) {
            ...
            estudio.save(function(err, ex) {
                if (err) {
                    return res.status(500).json({
                        message: 'Error guardado estudios.',
                        err
                    });
                }
                return res.json(ex);
    ...
    eliminar: function(req, res) {
        ...
        egresados.findOne({ _id: idcv }, function(err, cv) {
            if (err) {
                return res.status(500).json({
                    message: 'Error eliminar cv estudios.',
                    err
                });
            ...
            estudios.findByIdAndRemove(id, function(err, data) {
                if (err) {
                    return res.status(500).json({
                        message: 'Error error eliminar estudio.',
                        err
                    });
                }
                if (!data) {
